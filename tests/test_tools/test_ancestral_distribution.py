...
from biotaphy.tools.ancestral_distribution import cli
...
def test_package_valid(monkeypatch, valid_ancestral_distribution_package, tmpdir):
    ...
    Args:
        ...
        valid_ancestral_distribution_package (pytest.fixture): A pytest fixture that
            is parameterized to provide a valid ancestral distribution package, one at
            a time, and create new test functions for each.
        tmpdir (pytest.fixture): A built-in pytest fixture providing a
            temporary directory for this test function.

    Raises:
        IOError: When the tree or alignment cannot be loaded for the specified file
            extension.
        Exception: When a specified successful result value cannot be found.
    """
    # Get the data files
    ...
    for i in range(len(out_ml_results)):
        ...
        for i in range(len(test_ml_results)):
            if all(
                [
                    np.all(np.isclose(ml_row, test_ml_results[i])),
                    np.all(np.isclose(std_err_row, test_std_err_results[i]))
                ]
            ):
                found = True
